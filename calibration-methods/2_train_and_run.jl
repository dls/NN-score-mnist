...
RunResults() = DataFrame(net_output=Array{Float64,2}[],
                         ensemble_output=Array{Float64,1}[],
                         score=Float64[],
                         classified_category=Int64[],
...
function score_data(models, x, y)
  ...
  for i=1:size(res, 2)
    (max_score, idx) = findmax(res[:,i])
    push!(r, (copy(raw_res[:,:,i]), copy(res[:,i]), max_score, idx-1, y[i]))
  end
...
function score_test_data(models; kws...)
  ...
  r1 = score_data(models, test1_x, test1_y)
  r2 = score_data(models, test2_x, test2_y)
